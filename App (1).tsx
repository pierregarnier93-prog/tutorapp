      ...
      <div className="auth-box">
        ...
        {tab==="signup"&&(
          <div style={{fontSize:11,color:"#9CA3AF",textAlign:"center",marginTop:10,lineHeight:1.6}}>
            ...
            <span style={{color:"#5B4FE8",cursor:"pointer",textDecoration:"underline"}}>Terms of Service</span>,{" "}
            <span style={{color:"#5B4FE8",cursor:"pointer",textDecoration:"underline"}}>Privacy Policy</span>{" "}
            and <span style={{color:"#5B4FE8",cursor:"pointer",textDecoration:"underline"}}>Child Protection Charter</span>.
          </div>
        )}
      </div>
    </div>
...
export default function TutorApp() {
  ...
  return (
    <div className={`app-root${isRTL?" rtl":""}`}>
      ...
      {page==="legal"&&<div className="section" style={{maxWidth:800}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:"2rem",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {[
              {key:"tos",en:"Terms of Service",ar:"شروط الخدمة",fr:"CGU"},
              {key:"privacy",en:"Privacy Policy",ar:"سياسة الخصوصية",fr:"Confidentialité"},
              {key:"refund",en:"Refund Policy",ar:"سياسة الاسترداد",fr:"Remboursement"},
              {key:"tutor",en:"Tutor Agreement",ar:"اتفاقية المدرس",fr:"Contrat Enseignant"},
              {key:"child",en:"Child Protection",ar:"حماية القاصرين",fr:"Protection Mineurs"},
              {key:"legal",en:"Legal Notice",ar:"إشعار قانوني",fr:"Mentions Légales"},
            ].map(item=>(
              <button key={item.key}
                style={{padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,border:"1.5px solid",cursor:"pointer",transition:"all .2s",
                  borderColor:legalPage===item.key?"#5B4FE8":"#E8EAF6",
                  background:legalPage===item.key?"#5B4FE8":"#fff",
                  color:legalPage===item.key?"#fff":"#6B7280"}}
        ...
        {/* TERMS OF SERVICE */}
        {legalPage==="tos"&&<div style={{lineHeight:1.8,color:"#374151"}}>
          <h1 style={{fontFamily:"Fraunces,serif",fontSize:28,fontWeight:900,color:"#1A1A2E",marginBottom:8}}>
            {lang==="fr"?"Conditions Générales d'Utilisation":lang==="ar"?"شروط الخدمة":"Terms of Service"}
          </h1>
          <div style={{fontSize:13,color:"#9CA3AF",marginBottom:"2rem"}}>Version 1.0 — May 2025 · Orlixa Digital Commerce F.Z.C · Trade License 53175</div>
          {[
            {title:lang==="ar"?"طبيعة المنصة":"Nature of the Platform",content:lang==="ar"?"TutorApp هي منصة وساطة تربط الطلاب بالمدرسين المستقلين في منطقة الخليج. تعمل بصفة وسيط تقني فقط وليست صاحب عمل للمدرسين.":lang==="fr"?"TutorApp est une marketplace mettant en relation des Élèves avec des Enseignants indépendants dans la région du Golfe. Elle agit en qualité d'intermédiaire technique uniquement.":"TutorApp is a marketplace connecting Students with independent Tutors in the Gulf region. It acts as a technical intermediary only and is not the employer of Tutors."},
            {title:lang==="ar"?"القبول":"Acceptance",content:lang==="ar"?"باستخدام المنصة، تقبل هذه الشروط. استخدام القاصرين يستلزم موافقة ولي الأمر.":lang==="fr"?"En utilisant la plateforme, vous acceptez ces CGU. L'utilisation par des mineurs nécessite le consentement parental.":"By using the platform, you accept these Terms. Use by minors requires parental consent."},
            {title:lang==="ar"?"الأسعار والرسوم":"Pricing & Fees",content:lang==="ar"?"يحدد المدرسون أسعارهم بحرية. رسوم الخدمة 6% على الطالب + 6% على المدرس. إجمالي عمولة TutorApp 12%.":lang==="fr"?"Les Enseignants fixent librement leurs tarifs. Frais de service 6% côté Élève + 6% côté Enseignant. Commission totale TutorApp : 12%.":"Tutors set their own rates. 6% service fee for Students + 6% platform fee for Tutors. TutorApp total commission: 12%."},
            {title:lang==="ar"?"الدفع":"Payment",content:lang==="ar"?"يتم الدفع بعد إتمام الحصة عبر Stripe. لا يُجرى أي خصم قبل تأكيد إتمام الحصة.":lang==="fr"?"Le paiement est effectué après la réalisation du cours, via Stripe. Aucun débit avant confirmation.":"Payment is processed after the session via Stripe. No charge before the session is confirmed."},
            {title:lang==="ar"?"حماية القاصرين":"Protection of Minors",content:lang==="ar"?"التحقق من المدرسين إلزامي. الحصص عبر الإنترنت فقط. التواصل عبر المنصة فقط. عدم التسامح مع أي سلوك غير لائق.":lang==="fr"?"Vérification des Enseignants obligatoire. Cours en ligne uniquement. Communication via la plateforme uniquement. Tolérance zéro.":"Tutor verification mandatory. Online sessions only. Platform communication only. Zero tolerance for inappropriate behavior."},
            {title:lang==="ar"?"إلغاء واسترداد":"Cancellation & Refund",content:lang==="ar"?"إلغاء قبل 24 ساعة: استرداد كامل. إلغاء بعد 24 ساعة: استرداد 50%. غياب بدون إلغاء: لا استرداد.":lang==="fr"?"Annulation > 24h avant : remboursement total. < 24h : 50%. No-show : aucun remboursement.":"Cancellation > 24h: full refund. < 24h: 50%. No-show: no refund."},
            {title:lang==="ar"?"القانون المطبق":"Governing Law",content:lang==="ar"?"تخضع هذه الشروط لقوانين دولة الإمارات العربية المتحدة.":lang==="fr"?"Ces CGU sont soumises au droit des Émirats Arabes Unis.":"These Terms are governed by the laws of the United Arab Emirates."},
          ].map((section,i)=>(
            <div key={i} style={{marginBottom:"1.5rem",borderBottom:"1px solid #F4F5F7",paddingBottom:"1.5rem"}}>
              <h3 style={{fontFamily:"Fraunces,serif",fontSize:17,fontWeight:700,color:"#1A1A2E",marginBottom:8}}>{i+1}. {section.title}</h3>
              <p style={{fontSize:14,color:"#6B7280"}}>{section.content}</p>
            </div>
          ))}
          <div style={{background:"#EEF0FF",borderRadius:12,padding:"1rem",fontSize:13,color:"#3D34C4",fontWeight:600}}>
            📧 {lang==="ar"?"للتواصل:":lang==="fr"?"Contact :":"Contact:"} orlixa98@gmail.com
          </div>
        </div>}

        {/* PRIVACY POLICY */}
        {legalPage==="privacy"&&<div style={{lineHeight:1.8,color:"#374151"}}>
          <h1 style={{fontFamily:"Fraunces,serif",fontSize:28,fontWeight:900,color:"#1A1A2E",marginBottom:8}}>Privacy Policy</h1>
          <div style={{fontSize:13,color:"#9CA3AF",marginBottom:"2rem"}}>Version 1.0 — May 2025</div>
          {[
            {title:"Data We Collect",content:"Name, email, profile information (curriculum, level, subjects), payment data via Stripe only. We never store your card details."},
            {title:"Protection of Minors",content:"Minor data is never used for advertising or shared with third parties. Parental consent required for users under 16."},
            {title:"Data Sharing",content:"We share data only with Stripe (payments), Supabase (hosting), Jitsi (video), and legal authorities when required. We never sell your data."},
            {title:"Your Rights",content:"Right of access, rectification, deletion, opposition, and portability. Contact: orlixa98@gmail.com. Response within 30 days."},
            {title:"Data Retention",content:"Active account: throughout use. Session history: 3 years. Billing data: 7 years (UAE legal requirement)."},
            {title:"Security",content:"SSL/TLS encryption, secure authentication, restricted data access, regular encrypted backups."},
          ].map((section,i)=>(
            <div key={i} style={{marginBottom:"1.5rem",borderBottom:"1px solid #F4F5F7",paddingBottom:"1.5rem"}}>
              <h3 style={{fontFamily:"Fraunces,serif",fontSize:17,fontWeight:700,color:"#1A1A2E",marginBottom:8}}>{section.title}</h3>
              <p style={{fontSize:14,color:"#6B7280"}}>{section.content}</p>
            </div>
          ))}
        </div>}
        ...
        {/* REFUND POLICY */}
        {legalPage==="refund"&&<div style={{lineHeight:1.8}}>
          <h1 style={{fontFamily:"Fraunces,serif",fontSize:28,fontWeight:900,color:"#1A1A2E",marginBottom:8}}>Refund Policy</h1>
          <div style={{fontSize:13,color:"#9CA3AF",marginBottom:"2rem"}}>Version 1.0 — May 2025</div>
          <div style={{background:"#E6FAF8",border:"1.5px solid #0ABFA3",borderRadius:12,padding:"1rem",marginBottom:"1.5rem",fontSize:14,fontWeight:600,color:"#0F6E56"}}>
            💳 TutorApp operates on a pay-after-session basis. Your card is only charged after the session is confirmed as completed.
          </div>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}>
            <thead><tr style={{background:"#FAFBFF"}}>
              <th style={{padding:"10px 14px",textAlign:"left",borderBottom:"2px solid #E8EAF6",fontWeight:800}}>Situation</th>
              <th style={{padding:"10px 14px",textAlign:"left",borderBottom:"2px solid #E8EAF6",fontWeight:800}}>Refund</th>
            </tr></thead>
            <tbody>
              {[
                ["Student cancels > 24h before","✅ 100% refunded"],
                ["Student cancels < 24h before","⚠️ 50% refunded"],
                ["Student no-show","❌ No refund"],
                ["Tutor cancels","✅ 100% refunded"],
                ["Technical issue (Tutor side)","✅ 100% refunded"],
                ["Quality dispute (within 48h)","⚠️ Case by case"],
                ["Billing error","✅ 100% refunded"],
              ].map(([sit,ref],i)=>(
                <tr key={i} style={{borderBottom:"1px solid #F4F5F7"}}>
                  <td style={{padding:"10px 14px",color:"#374151"}}>{sit}</td>
                  <td style={{padding:"10px 14px",fontWeight:700}}>{ref}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{marginTop:"1.5rem",fontSize:13,color:"#6B7280"}}>
            To request a refund: <strong>orlixa98@gmail.com</strong> — Subject: [REFUND] + booking reference
          </div>
        </div>}

        {/* TUTOR AGREEMENT */}
        {legalPage==="tutor"&&<div style={{lineHeight:1.8,color:"#374151"}}>
          <h1 style={{fontFamily:"Fraunces,serif",fontSize:28,fontWeight:900,color:"#1A1A2E",marginBottom:8}}>Tutor Agreement</h1>
          <div style={{fontSize:13,color:"#9CA3AF",marginBottom:"2rem"}}>Version 1.0 — May 2025</div>
          {[
            {title:"Tutor Status",content:"Tutors operate as independent contractors. No employment relationship exists with TutorApp. Tutors are responsible for their own tax and administrative obligations."},
            {title:"Eligibility",content:"Must be 18+, provide valid government ID and proof of qualifications, have no criminal record related to offenses against minors, and have stable internet."},
            {title:"Commission",content:"TutorApp deducts 6% per session. Tutors receive 94% of their displayed rate. Example: 300 AED rate → Tutor receives 282 AED."},
            {title:"Obligations Regarding Minors ⚠️",content:"Maintain strictly professional communication. Never contact minors outside the platform. Never record sessions without consent. Immediately report any concerning behavior. Zero tolerance — any breach results in immediate permanent suspension."},
            {title:"Non-Circumvention",content:"Tutors agree not to contact Students outside the platform during registration and for 12 months after termination. Violation results in a 3-month commission penalty."},
            {title:"Cancellation",content:"Cancellation < 24h: warning. 3rd warning: temporary suspension. No-show: 7-day suspension."},
          ].map((section,i)=>(
            <div key={i} style={{marginBottom:"1.5rem",borderBottom:"1px solid #F4F5F7",paddingBottom:"1.5rem"}}>
              <h3 style={{fontFamily:"Fraunces,serif",fontSize:17,fontWeight:700,color:"#1A1A2E",marginBottom:8}}>{section.title}</h3>
              <p style={{fontSize:14,color:"#6B7280"}}>{section.content}</p>
        ...
        {/* CHILD PROTECTION */}
        {legalPage==="child"&&<div style={{lineHeight:1.8}}>
          <h1 style={{fontFamily:"Fraunces,serif",fontSize:28,fontWeight:900,color:"#1A1A2E",marginBottom:8}}>Child Protection Charter</h1>
          <div style={{fontSize:13,color:"#9CA3AF",marginBottom:"2rem"}}>Version 1.0 — May 2025</div>
          <div style={{background:"#FEE2E2",border:"1.5px solid #FCA5A5",borderRadius:12,padding:"1rem",marginBottom:"1.5rem",fontSize:14,fontWeight:700,color:"#B91C1C"}}>
            🚫 ZERO TOLERANCE — Any inappropriate behavior toward a minor results in immediate permanent suspension and may be reported to UAE authorities.
          </div>
          {[
            {title:"For Tutors — What you MUST do",content:"Maintain professional communication at all times. Use only platform tools. Immediately report any concerning behavior. Conduct sessions in an appropriate environment. Obtain parental consent before any recording."},
            {title:"For Tutors — What is STRICTLY PROHIBITED",content:"Contacting a minor outside the platform. Requesting personal information beyond session needs. Sharing inappropriate content. Recording without consent. Arranging any in-person meeting."},
            {title:"For Parents — Your Rights",content:"Supervise your child's sessions. Report inappropriate behavior to orlixa98@gmail.com. Request deletion of your child's data at any time. Access your child's session history."},
            {title:"TutorApp Protection Measures",content:"All Tutors verified (ID + qualifications) before access. Online sessions only. All communications on platform. Rating system after each session. Reports handled within 24 hours."},
            {title:"Emergency Contacts",content:"UAE Police: 999 | UAE Child Helpline: 116 | TutorApp: orlixa98@gmail.com — [CHILD SAFETY REPORT]"},
          ].map((section,i)=>(
            <div key={i} style={{marginBottom:"1.5rem",borderBottom:"1px solid #F4F5F7",paddingBottom:"1.5rem"}}>
              <h3 style={{fontFamily:"Fraunces,serif",fontSize:17,fontWeight:700,color:"#1A1A2E",marginBottom:8}}>{section.title}</h3>
              <p style={{fontSize:14,color:"#6B7280"}}>{section.content}</p>
        ...
        {/* LEGAL NOTICE */}
        {legalPage==="legal"&&<div style={{lineHeight:1.8,color:"#374151"}}>
          <h1 style={{fontFamily:"Fraunces,serif",fontSize:28,fontWeight:900,color:"#1A1A2E",marginBottom:"2rem"}}>Legal Notice</h1>
          {[
            {label:"Company",value:"Orlixa Digital Commerce F.Z.C"},
            {label:"Legal Form",value:"Free Zone Company (F.Z.C)"},
            {label:"Trade License",value:"53175 — Ajman Free Zone Authority (AFZA)"},
            {label:"Address",value:"C1 Building, Premises No. BC 1305672, Ajman Free Zone, Ajman, UAE"},
            {label:"Email",value:"orlixa98@gmail.com"},
            {label:"Legal Representative",value:"Sarah Amimeur Garnier"},
            {label:"Frontend Hosting",value:"Vercel Inc., San Francisco, CA, USA"},
            {label:"Database",value:"Supabase Inc., Singapore"},
            {label:"Payments",value:"Stripe Inc., San Francisco, CA, USA"},
            {label:"Governing Law",value:"United Arab Emirates"},
          ].map((row,i)=>(
            <div key={i} style={{display:"flex",gap:16,padding:"10px 0",borderBottom:"1px solid #F4F5F7",flexWrap:"wrap"}}>
              <span style={{fontWeight:700,minWidth:160,color:"#1A1A2E",fontSize:14}}>{row.label}</span>
              <span style={{color:"#6B7280",fontSize:14}}>{row.value}</span>
            </div>
          ))}
        </div>}
      </div>}
      ...
      <footer className="footer">
        ...
        <div style={{display:"flex",gap:"1.5rem",justifyContent:"center",flexWrap:"wrap",marginTop:"1.5rem"}}>
          {[
            ...
            <span key={item.key} style={{fontSize:12,color:"#9CA3AF",cursor:"pointer",textDecoration:"underline"}}
              onClick={()=>{setLegalPage(item.key);setPage("legal");}}>
              {item[lang]||item.en}
            </span>
          ))}
        </div>
        <div style={{marginTop:"1rem",fontSize:12,color:"#4B5563"}}>© 2025 TutorApp · Orlixa Digital Commerce F.Z.C · Trade License 53175 · Ajman Free Zone</div>
      </footer>
